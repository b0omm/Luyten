...
public class Model extends JFrame implements WindowListener {
    ...
    //
    // I'm caching this for performance reasons. There is a pretty substantial
    // cost associated
    // with loading types from the disk. If you cache the metadata,
    // decompilation should be much
    // faster after the first few classes.
    //
    // Ideally, this should be purged if the user elects to 'refresh' the view.
    // It'll retain
    // metadata for types it has already seen, so if the user replaces the
    // classes/jars they're
    // viewing and wants to see changes, a refresh feature that purges the
    // MetadataSystem would
    // be useful.
    //
    ...
    public void setup() {
        ...
        a = new JRadioButtonMenuItem(new ThemeAction("Default", "default.xml"));
        ...
        if (settings.getFormattingOptions() == null) settings.setFormattingOptions(JavaFormattingOptions.createDefault());
            ...
            public void run() {
                if (args.length > 0) {
                    if (Arrays.asList(args).contains("--nogui")) {
                        ...
                        list.remove("--nogui");
                        DecompilerDriver.main(list.toArray(new String[list.size()]));
                    } else {
                        new Model(args[0]);
                    }
                } else {
    ...
    public void addTab(String title, RTextScrollPane rTextScrollPane) {
        if (house.indexOfTab(title) < 0) {
            ...
            ct.addMouseListener(new CloseTab(title,true));
    ...
    public void closeOpenTab(int index) {
        ...
        for (OpenFile file : hmap)
            if (pane.equals(file.textArea)) open = file;
        if (open != null && hmap.contains(open)) hmap.remove(open);
        house.remove(co);
    }
    ...
    private class TreeListener extends MouseAdapter {
        ...
        public void mouseClicked(MouseEvent event) {
            ...
            if (trp != null) {
                DefaultMutableTreeNode tn = (DefaultMutableTreeNode) trp.getLastPathComponent();
                if (SwingUtilities.isLeftMouseButton(event) && event.getClickCount() == 2) {
                    ...
                    try {
                        if (args.length > 1) {
                            ...
                            //
                            // Note: You shouldn't ever need to set this. It's only
                            // for languages that support catch
                            // blocks without an exception variable. Java doesn't
                            // allow this. I think Scala does.
                            //
                            ...
                            if (file.getName().endsWith(".jar") || file.getName().endsWith(".zip")) {
                                try {
                                    ...
                                    if (entry.getName().endsWith(".class")) {
                                        ...
                                        settings.getLanguage().decompileType(resolvedType, new PlainTextOutput(stringwriter),
    ...
    private class CloseTab extends MouseAdapter {
        ...
        public CloseTab(String title, boolean isCenter) {
            this.title = title;
        ...
        public void mouseClicked(MouseEvent e) {
            if (isCenter && e.getButton() != MouseEvent.BUTTON2) {
                house.setSelectedIndex(house.indexOfTab(title));
                return;
            }
            int index = house.indexOfTab(title);
    ...
    private class FileLoad implements ActionListener {
        ...
        public void actionPerformed(ActionEvent e) {
            ...
            if (returnVal == JFileChooser.APPROVE_OPTION) {
                ...
                if (open) new FileClose().actionPerformed(e);
                loadFile(file);
            }
        }
                    ...
                    if (file.getName().endsWith(".zip") || file.getName().endsWith(".jar")) {
                        ...
                        try {
                            ...
                            while (entry.hasMoreElements()) {
                                ...
                                if (!e.isDirectory()) mass.add(e.getName());

                            }
                            List<String> sort = new ArrayList<String>();
                            Collections.sort(mass, String.CASE_INSENSITIVE_ORDER);
                            for (String m : mass)
                                if (m.contains("META-INF") && !sort.contains(m)) sort.add(m);
                            ...
                            List<String> packs = Arrays.asList(set.toArray(new String[set.size()]));
                            ...
                            for (String pack : packs)
                                for (String m : mass)
                                    if (!m.contains("META-INF") && m.contains(pack) && !m.replace(pack, "").contains("/")) sort.add(m);
                            for (String m : mass)
                                if (!m.contains("META-INF") && !m.contains("/") && !sort.contains(m)) sort.add(m);
    ...
    private class FileExtractJar implements ActionListener {
        ...
        public void actionPerformed(ActionEvent e) {
            ...
            if (s.toLowerCase().endsWith(".jar")) s = s.replaceAll("\\.[jJ][aA][rR]", ".zip");
                        ...
                        try {
                            ...
                            while (ent.hasMoreElements()) {
                                ...
                                if (entry.getName().endsWith(".class")) {
                                    ...
                                    settings.getLanguage().decompileType(resolvedType, new PlainTextOutput(new OutputStreamWriter(out)),
                                            decompilationOptions);
                                    out.closeEntry();
                                } else {
    ...
    public class FileChooserFileFilter extends FileFilter {
        ...
        public boolean accept(File f) {
            return !f.isDirectory() && f.getName().toLowerCase().endsWith(objType.substring(1));
        }
                ...
                for (DataFlavor flavor : flavors) {
                    try {
                        if (flavor.isFlavorJavaFileListType()) {
                            ...
                            if (files.size() == 1) {
                                if (open) new FileClose().actionPerformed(null);
                                file = files.get(0);
                                new FileLoad(false).loadFile(file);
                            }
                        }
    ...
    private class ThemeAction extends AbstractAction {
        ...
        public ThemeAction(String name, String xml) {
            putValue(NAME, name);
            this.xml = "/themes/" + xml.toLowerCase();
        }
